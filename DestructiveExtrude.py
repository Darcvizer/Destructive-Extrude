...
def Duplicate(self, context, act_obj):
	...
	bpy.ops.mesh.offset_edges(geometry_mode = 'move', width = 0.02)

	bm = bmesh.from_edit_mesh(act_obj.data)



	#verts_for_delete = [v.index  for v in bm.verts if v.select]
	...
	#Dub = [f for i in bm.faces if i.select]
	bm.verts.ensure_lookup_table()
	bm.faces.ensure_lookup_table()
